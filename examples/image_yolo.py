...
Note: Install easy-drone first with: pip install -e .[yolo]
"""
import argparse
import json
import os
...
try:
    ...
    print("[YOLO] ERROR: easy-drone is not installed.")
    print("Install easy-drone with YOLO dependencies: pip install -e .[yolo]")
    raise
...
def serialize_detection2d_array_cdr(
    ...
    buf.extend(struct.pack('<I', len(frame_id_bytes) + 1))
    ...
    for det in detections:
        ...
        # frame_id length (empty string = 1 for null)
        ...
        # pose (x, y, z, qx, qy, qz, qw) + covariance
        buf.extend(b'\x00' * (7 * 8 + 36 * 8))
...
class YoloPublisher:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        with open(self.classes_path, 'r') as f:
            ...
            self.class_index_to_name = {
                int(k): v for k, v in class_map.items()}

        # Generate colors for visualization
        import matplotlib.pyplot as plt
        colors_rgba = plt.cm.hsv(np.linspace(
            0, 1, len(self.class_index_to_name)))
        ...
        if args.zenoh_connect:
            zenoh_config.insert_json5(
                "connect/endpoints", f'["{args.zenoh_connect}"]')
    ...
    def _on_gz_image(self, msg: Image) -> None:
        ...
        try:
            ...
            if not hasattr(self, '_printed_format'):
                print(
                    f"[YOLO] Image format: {msg.pixel_format_type} (RGB_INT8={image_pb2.RGB_INT8}, BGR_INT8={image_pb2.BGR_INT8})")
                print(
                    f"[YOLO] Image size: {msg.width}x{msg.height}, data: {len(msg.data)} bytes")
            ...
            if msg.pixel_format_type == image_pb2.RGB_INT8:
                ...
                print(
                    f"[YOLO] WARNING: Unsupported pixel format: {msg.pixel_format_type}")
                return
    ...
    def _watchdog_check(self) -> None:
        ...
        if self._frame_count == 0:
            ...
            print(
                f"   - Verify publisher visible: gz topic -i -t {self.gz_topic}")
        ...
        try:
            while True:
                try:
                    ...
                    print(
                        '[YOLO] WARNING: Frame queue empty - no frames received in 1 second')
                    print('[YOLO] Troubleshooting steps:')
                    print(
                        f'  1. Verify topic is active: gz topic -i -t {self.gz_topic}')
                    print(
                        f'  2. Echo messages: gz topic -e -t {self.gz_topic}')
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--gz-topic', required=True,
                        help='Gazebo image topic (e.g., /camera)')
    parser.add_argument('--model', type=str, required=True,
                        help='Path to ONNX model')
    parser.add_argument('--classes', type=str, required=True,
                        help='Path to COCO classes JSON')
    parser.add_argument('--input-size', type=int,
                        default=640, help='Model input size')
    ...
    parser.add_argument('--nms-threshold', type=float,
                        default=0.45, help='NMS threshold')
    parser.add_argument('--zenoh-topic', type=str,
                        default='rt/detections', help='Zenoh topic for detections')
    parser.add_argument('--zenoh-mode', type=str,
                        default='peer', help='Zenoh mode: peer or client')
    parser.add_argument('--zenoh-connect', type=str, default='',
                        help='Zenoh router endpoint (e.g., tcp/42.42.1.1:7447)')
    parser.add_argument('--visualize', action='store_true',
                        help='Show visualization window with detections')
