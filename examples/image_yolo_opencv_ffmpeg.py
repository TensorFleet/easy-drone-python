...
class YoloPublisherOpenCVFFMPEG(YoloPublisher):
    ...
    def _open_video_capture(self) -> None:
        ...
        if self.input_is_gstreamer:
            ...
            print(
                f"[YOLO] Attempting to open UDP stream on port {self.udp_port} using OpenCV+FFMPEG...")
                ...
                try:
                    ...
                    if cap.isOpened():
                        ...
                        if ret and frame is not None:
                            print(
                                f"[YOLO] UDP stream opened successfully using OpenCV+FFMPEG!")
                            print(f"[YOLO] Frame size: {frame.shape}")
                            self._capture = cap
                            return
                        else:
                            print(
                                f"[YOLO] Stream opened but could not read frame")
                            cap.release()
                    else:
            ...
            print(
                f"     ffplay -fflags nobuffer udp://127.0.0.1:{self.udp_port}")
            print(f"")
